...
import { BusRoute, BusStop, DijkstraResult, MetricType, Position } from "@/types/graph";
...
interface GraphCanvasProps {
  nodes: BusStop[];
  edges: BusRoute[];
  onNodeAdd: (node: BusStop) => void;
  onNodeMove: (id: string, position: Position) => void;
  onNodeDelete: (id: string) => void;
  onEdgeAdd: (edge: BusRoute) => void;
...
const GraphCanvas: React.FC<GraphCanvasProps> = ({
  ...
  const handleCanvasClick = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    // If clicking on the background, add a new node
    ...
    if (!clickedOnNode && e.target === e.currentTarget) {
      const newNodeId = `node-${Date.now()}`;
      onNodeAdd({
        id: newNodeId,
        label: `Stop ${nodes.length + 1}`,
        position
      });
      
      toast("Bus stop added successfully", {
        description: "Click and drag to move it or double-click to delete."
      });
    }
  };

  // Handle real-world location selection
      ...
      if (connectingFrom !== nodeId) {
        ...
        if (!edgeExists) {
          ...
          if (sourceNode && targetNode) {
            let cost = Math.random() * 10 + 5;
            
            // If both nodes have real-world coordinates, calculate distance
            if (sourceNode.metadata?.lat && targetNode.metadata?.lat) {
              // Calculate haversine distance
              const R = 6371; // Earth's radius in km
              const dLat = (targetNode.metadata.lat - sourceNode.metadata.lat) * Math.PI / 180;
              const dLon = (targetNode.metadata.lon - sourceNode.metadata.lon) * Math.PI / 180;
              const a = 
                Math.sin(dLat/2) * Math.sin(dLat/2) +
                Math.cos(sourceNode.metadata.lat * Math.PI / 180) * Math.cos(targetNode.metadata.lat * Math.PI / 180) * 
                Math.sin(dLon/2) * Math.sin(dLon/2);
              const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
              cost = R * c; // Distance in km
            }
            ...
            toast("Connection added", {
              description: "A new route between stops has been created."
            });
          }
        } else {
          toast("Connection already exists", {
            description: "These stops are already connected."
          });
        }
      }
  ...
  const handleNodeDoubleClick = (e: React.MouseEvent, nodeId: string) => {
    ...
    toast("Bus stop deleted", {
      description: "The stop and all its connections have been removed."
    });
  };
  ...
  const handleEdgeDoubleClick = (e: React.MouseEvent, edgeId: string) => {
    ...
    toast("Connection deleted", {
      description: "The route between stops has been removed."
    });
  };
  ...
  const handleStartConnecting = (e: React.MouseEvent, nodeId: string) => {
    ...
    toast("Creating connection", {
      description: "Click on another stop to create a route between them."
    });
  };

  // Calculate edge path
  const getEdgePath = (source: BusStop, target: BusStop): string => {
  ...
  const getEdgeLabelPosition = (source: BusStop, target: BusStop) => {
    return {
  ...
  const isEdgeInPath = (edge: BusRoute): boolean => {
    if (!result || !result.path || result.path.length < 2 || visualSteps === -1) {
      return false;
    }
  ...
  const isNodeVisited = (nodeId: string): boolean => {
    if (!result || visualSteps === -1) return false;
    
    const currentStep = result.steps[visualSteps];
    return currentStep.visitedNodes.includes(nodeId);
  };
  ...
  const isCurrentNode = (nodeId: string): boolean => {
    ...
    const currentStep = result.steps[visualSteps];
    return currentStep.currentNode === nodeId;
  };

  // Get node CSS classes based on its state
  const getNodeClasses = (node: BusStop) => {
    const classes = ["node"];
    ...
    if (node.metadata?.realWorld) classes.push("node-real-world");
    
    return classes.join(" ");
  };

  // Get edge CSS classes based on its state
  const getEdgeClasses = (edge: BusRoute) => {
    const classes = ["edge"];
    
    if (isEdgeInPath(edge)) classes.push("edge-active animate-trace-path");
    else if (visualSteps !== -1 && 
  ...
  // Toggle between abstract and real-world map
  const toggleRealWorldMap = () => {
    setUseRealWorldMap(!useRealWorldMap);
  };
  ...
  return (
    <div className="relative w-full h-full bg-muted rounded-lg overflow-hidden">
      <div className="absolute top-4 right-4 flex gap-2 z-10">
        ...
        <button 
          className="px-3 py-1 bg-primary text-primary-foreground text-sm rounded-full shadow-sm hover:bg-primary/80 transition-colors"
          onClick={toggleRealWorldMap}
        >
          {useRealWorldMap ? "Abstract Mode" : "Real World Mode"}
        </button>
      </div>
      
      {useRealWorldMap && (
        <div className="absolute top-14 right-4 z-10 w-64 bg-card p-3 rounded-lg shadow-md">
          <h3 className="text-sm font-medium mb-2">Add Real Location</h3>
          <LocationSearch 
      ...
      {connectingFrom && (
        <div className="absolute top-4 left-4 py-2 px-4 bg-card shadow-md rounded-lg z-10 animate-fade-in">
          <p className="text-sm">Click on another bus stop to connect</p>
          <button 
      ...
      >
        {/* Map Background for Real World Mode */}
        {useRealWorldMap && (
          <image
            href={`https://tile.openstreetmap.org/7/64/42.png`}
            width="100%"
            height="100%"
            opacity="0.4"
        ...
        {/* Grid Background for Abstract Mode */}
        {!useRealWorldMap && (
          <>
        ...
        {edges.map(edge => {
          ...
          return (
            <g key={edge.id} onDoubleClick={(e) => handleEdgeDoubleClick(e, edge.id)}>
              <path
                d={path}
                strokeWidth={2}
        ...
        {nodes.map(node => (
          <g 
            ...
            style={{ cursor: isDragging && selectedNodeId === node.id ? 'grabbing' : 'pointer' }}
          >
            {/* Node Circle */}
            <circle
              r={NODE_RADIUS}
            ...
            {/* Connect button */}
            {selectedNodeId === node.id && !animationInProgress && !connectingFrom && (
              <g 
      ...
      {nodes.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center bg-background/50 backdrop-blur-sm z-10">
          <div className="max-w-md p-6 bg-card rounded-lg shadow-lg text-center">
            ...
            {useRealWorldMap ? (
              <>
                <p className="mb-3">Use the search box in the top right to add real-world locations.</p>
                <p className="mb-3">Connect stops by selecting one and clicking the + button.</p>
                <p className="mb-3">Hold Shift and click to set a start point.</p>
                <p className="mb-3">Hold Alt and click to set a destination.</p>
              </>
            ) : (
              <>
                <p className="mb-3">Click anywhere on the canvas to add bus stops.</p>
                <p className="mb-3">Connect stops by selecting one and clicking the + button.</p>
                <p className="mb-3">Hold Shift and click to set a start point.</p>
                <p className="mb-3">Hold Alt and click to set a destination.</p>
              </>
            ...
            <button 
              ...
              onClick={() => {
                const demoNodes = [
                  { id: 'node-1', label: 'Stop 1', position: { x: 200, y: 200 } },
                  { id: 'node-2', label: 'Stop 2', position: { x: 400, y: 150 } },
                  { id: 'node-3', label: 'Stop 3', position: { x: 600, y: 200 } },
                  { id: 'node-4', label: 'Stop 4', position: { x: 300, y: 350 } },
                  { id: 'node-5', label: 'Stop 5', position: { x: 500, y: 350 } },
                ];
                
                const demoEdges = [
                  { id: 'edge-1', source: 'node-1', target: 'node-2', cost: 5.0, time: 2.5, distance: 4.0 },
                  { id: 'edge-2', source: 'node-2', target: 'node-3', cost: 7.0, time: 3.5, distance: 5.6 },
                  { id: 'edge-3', source: 'node-1', target: 'node-4', cost: 4.5, time: 2.3, distance: 3.6 },
                  { id: 'edge-4', source: 'node-4', target: 'node-5', cost: 6.2, time: 3.1, distance: 5.0 },
                  { id: 'edge-5', source: 'node-2', target: 'node-5', cost: 8.5, time: 4.3, distance: 6.8 },
                  { id: 'edge-6', source: 'node-3', target: 'node-5', cost: 6.8, time: 3.4, distance: 5.4 },
                ];
                ...
                toast("Demo map loaded", {
                  description: "Now you can run the algorithm to find the best route!"
                });
              }}
            >
              Load Demo Map
